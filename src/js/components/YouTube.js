...
class YouTube extends React.Component {
 ...
 /**
  ...
  onPlayerStateChange = (event) => {
    ...
    this.checkCurrentTime(event.target);
  ...
  checkCurrentTime = (player) =>{
      ...
      let updateTime = (player) => {
        if(this._remaining_time !== player.getCurrentTime()){
          this.onChangeCurrentTime(player.getCurrentTime());
          this._remaining_time = player.getCurrentTime();
        }
      }

      if(!this._timer){
        this._remaining_time = 0
        this._timer = setInterval(updateTime.bind(this, player), 100);
      }
  }
   ...
   */
  onChangeCurrentTime = time => this.props.onChangeCurrentTime(time);
